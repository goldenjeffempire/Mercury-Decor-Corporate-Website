...
- Build emits 29 distinct HTML routes with content, one H1, unique title and description, internal navigation and, when a verified production HTTPS origin is set, canonical URLs, Open Graph/Twitter metadata, Schema.org JSON-LD and an absolute-URL sitemap. Structured data describes the business, website, breadcrumb trails, services, curated images and catalog videos; it makes no offers, review or rating claims.
...
1. **Publish to a verified HTTPS production domain.** Set `VITE_SITE_URL` to its exact origin, for example `https://your-owned-domain.example` (no trailing slash, path or development domain). Rebuild/re-publish. The unpublished build intentionally uses `noindex` and does **not** generate a sitemap or production canonical URLs; it will not be index-ready until this setting is provided. Do not use a guessed `.replit.dev` host or a temporary example domain.
2. After publishing, visit the public `/robots.txt`, `/sitemap.xml`, homepage, one service, one category, one project, `/port-harcourt`, `/contact` and an unknown path. Confirm all preferred pages return HTTP 200 with HTTPS self-canonicals, unknown paths return HTTP 404, images and videos load, and no page source contains `localhost`, an example domain or `noindex`.
...
5. Check that other legitimate mentions/directories use the same NAP. Avoid bulk low-quality directory submissions or bought links. If the business has project permission and original documentation, add evidence-rich case studies and helpful local pages, not repeated city-name permutations.

## QA performed before publication

A production-style test build using an **example.org test origin, never intended for deployment**, checked 29 HTML routes, 29 unique titles, 29 unique descriptions, canonical consistency, schema JSON parsing, 182 internal links, 22 static preview images and all sitemap entries; it reported zero issues. The workspace was then rebuilt without the test origin to avoid shipping a fabricated URL. Desktop and mobile screenshots of a project and catalog category rendered without browser errors. This does **not** measure live search performance or replace Search Console verification.
