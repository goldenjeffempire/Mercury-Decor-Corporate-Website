  ...
  return <section id="showroom" className="overflow-hidden bg-[#061f3d] py-20 text-white md:py-28">
    <div className="container-wide">
      ...
      <div className="mt-8 grid gap-3 lg:grid-cols-[1.65fr_.7fr]">
        ...
        >
          <img key={active.src} src={active.src} alt={active.alt} draggable={false} decoding="async" onError={(event) => { event.currentTarget.style.display = 'none'; }} className="showroom-image pointer-events-none absolute inset-0 h-full w-full object-contain" />
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-1">
          {nextImages.map((image, position) => <button key={image.src} type="button" onClick={() => goTo(image.index)} aria-label={`Show image ${image.index + 1}`} className="reveal group relative min-h-[180px] overflow-hidden bg-[#0b3769] text-left sm:min-h-[240px]"><img src={image.src} alt={image.alt} loading="lazy" decoding="async" onError={(event) => { event.currentTarget.style.display = 'none'; }} className="absolute inset-0 h-full w-full object-contain" /><ImageWatermark /><span className="absolute inset-0 bg-gradient-to-t from-[#061f3d]/75 via-transparent to-transparent" /><span className="absolute bottom-4 left-4 z-20 text-xs font-bold uppercase tracking-[.12em] text-white/85">{position === 0 ? 'Up next' : 'Following'}</span></button>)}
        </div>
      </div>
...
function About() {
  return <Shell title="About Mercury Décor Limited | Nigerian Finishing Specialists" path="/about"><PageHero eyebrow="About Mercury Décor Limited" title="A clear standard for every" italic="surface." text="Professional interior and exterior finishing, construction, procurement, supply and project support for residential, commercial and corporate projects." /><section className="grid-blueprint py-24 md:py-32"><div className="container-wide grid gap-14 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:gap-24"><div className="reveal grid grid-cols-2 gap-3"><div className="relative col-span-2 overflow-hidden bg-[#dce4ec]"><img src={galleryImages[0].src} alt={galleryImages[0].alt} loading="lazy" className="aspect-[4/3] w-full object-contain" /><ImageWatermark /></div><div className="relative overflow-hidden bg-[#dce4ec]"><img src={galleryImages[5].src} alt={galleryImages[5].alt} loading="lazy" className="aspect-square w-full object-contain" /><ImageWatermark /></div><div className="relative overflow-hidden bg-[#dce4ec]"><img src={galleryImages[10].src} alt={galleryImages[10].alt} loading="lazy" className="aspect-square w-full object-contain" /><ImageWatermark /></div></div><div className="reveal reveal-delay-1"><SectionLabel>The company</SectionLabel><h2 className="mt-5 font-display text-4xl font-extrabold leading-[1.04] tracking-[-.05em] text-[#082b54] md:text-6xl">Practical solutions.<br /><span className="font-editorial font-semibold italic text-[#a90000]">Professional delivery.</span></h2><div className="mt-7 max-w-[570px] space-y-5 text-[15px] leading-7 text-[#4b5563]"><p>Mercury Décor Limited is a Nigerian company specializing in professional interior and exterior finishing, construction, procurement, supply and project support services.</p><p>We provide practical solutions for residential, commercial and corporate projects, combining quality workmanship, reliable materials and professional service delivery.</p></div><div className="mt-9 flex flex-wrap gap-3"><QuoteButton /><WhatsAppButton /></div></div></div></section></Shell>;
}
...
function ServiceCard({ service, image, index }: { service: (typeof services)[number]; image: (typeof showroomImages)[number]; index: number }) {
  ...
  return <article className={`reveal reveal-delay-${(index % 3) + 1} group flex flex-col overflow-hidden border border-[#082b54]/12 bg-white shadow-[0_18px_45px_rgba(8,43,84,.07)] transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_26px_70px_rgba(8,43,84,.15)]`}>
    <div className="relative aspect-[4/3] overflow-hidden bg-[#dce4ec]">
      <img src={image.src} alt={`${service.title} visual example`} loading="lazy" decoding="async" className="h-full w-full object-contain" />
      <ImageWatermark />
    ...
    <section className="grid-blueprint bg-white py-20 md:py-28">
      <div className="container-wide">
        ...
        <div className="mt-12 grid grid-cols-2 gap-3 lg:grid-cols-[1.35fr_.65fr_.65fr]">
          {[serviceVisuals[0], serviceVisuals[2], serviceVisuals[5]].map((image, index) => <figure key={image.src} className={`reveal relative overflow-hidden bg-[#dce4ec] ${index === 0 ? 'col-span-2 min-h-[360px] lg:col-span-1 lg:row-span-1' : 'min-h-[220px] lg:min-h-[360px]'}`}><img src={image.src} alt={image.alt} loading="lazy" decoding="async" className="absolute inset-0 h-full w-full object-contain" /><ImageWatermark large={index === 0} top /><figcaption className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#061f3d]/85 to-transparent px-5 pb-5 pt-20 text-[10px] font-bold uppercase tracking-[.12em] text-white">{index === 0 ? 'Finishing and decorative capability' : index === 1 ? 'Built environment support' : 'Corporate and commercial vision'}</figcaption></figure>)}
        </div>
      </div>
    </section>
